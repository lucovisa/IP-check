...
DNS: ${dns.hostname || ""}`;
    ...
    result.innerHTML = `
      ...
      ${field("Экран монитора", `${screenRes} <span class="screen-help" data-tooltip="Мы определили разрешение экрана с учетом плотности пикселей (retina), но без учета масштаба (zoom) в браузере. Если масштаб в браузере отличен от 100%, то разрешение может быть измерено неверно.">?</span>`)}
